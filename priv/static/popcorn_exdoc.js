...
async function runIexInput(code, promptSpan, blockId) {
  if (promptSpan.dataset.codeState === "EVALUATING" || promptSpan.dataset.codeState === "EVALUATED")
    return;
...
function decorateIexBlocks() {
  for (const codeEl of document.querySelectorAll("pre.popcorn-iex code")) {
    ...
    for (const line of lines) {
      ...
      wrapper.className = hasPrompt ? "popcorn-iex-line" : hasContinuation ? "popcorn-iex-continuation" : "output";
      wrapper.append(...line);
