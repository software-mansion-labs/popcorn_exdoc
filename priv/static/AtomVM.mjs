...
function ensureValidResult(result) {
  ...
  const message = "Evaluated script returned invalid value. Expected number array or null";
...
function ___syscall_mknodat(dirfd, path, mode, dev) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(8, 0, 1, dirfd, path, mode, dev);
  try {
...
function ___syscall_pipe(fdPtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(10, 0, 1, fdPtr);
  try {
    if (fdPtr == 0) {
...
var registerBatteryEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
  ...
  var batteryEventHandlerFunc = (e = event) => {
    ...
    if (targetThread) __emscripten_run_callback_on_thread(targetThread, callbackfunc, eventTypeId, batteryEvent, userData); else if (((a1, a2, a3) => dynCall_iiii(callbackfunc, a1, a2, a3))(eventTypeId, batteryEvent, userData)) e.preventDefault();
  };
  var eventHandler = {
...
function _emscripten_set_batterychargingchange_callback_on_thread(userData, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, userData, callbackfunc, targetThread);
  if (!battery()) return -1;
...
function _emscripten_set_batterylevelchange_callback_on_thread(userData, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, userData, callbackfunc, targetThread);
  if (!battery()) return -1;
...
function _emscripten_set_beforeunload_callback_on_thread(userData, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 0, 1, userData, callbackfunc, targetThread);
...
function _emscripten_set_devicemotion_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 0, 1, userData, useCapture, callbackfunc, targetThread);
...
var registerDeviceOrientationEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
  ...
  var deviceOrientationEventHandlerFunc = (e = event) => {
    fillDeviceOrientationEventData(JSEvents.deviceOrientationEvent, e);
    // TODO: Thread-safety with respect to emscripten_get_deviceorientation_status()
    if (targetThread) {
      var deviceOrientationEvent = _malloc(32);
      fillDeviceOrientationEventData(deviceOrientationEvent, e);
...
function _emscripten_set_deviceorientation_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, userData, useCapture, callbackfunc, targetThread);
...
var registerPointerlockErrorEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
  ...
  var pointerlockErrorEventHandlerFunc = (e = event) => {
    if (targetThread) __emscripten_run_callback_on_thread(targetThread, callbackfunc, eventTypeId, 0, userData); else if (((a1, a2, a3) => dynCall_iiii(callbackfunc, a1, a2, a3))(eventTypeId, 0, userData)) e.preventDefault();
  };
  var eventHandler = {
    target,
    eventTypeString,
...
function fetchXHR(fetch, onsuccess, onerror, onprogress, onreadystatechange) {
  ...
  var xhr = new XMLHttpRequest;
  xhr.withCredentials = !!SAFE_HEAP_LOAD((fetch_attr) + (60), 1, 1);
  ...
  SAFE_HEAP_STORE(((fetch) >> 2) * 4, id, 4);
  checkInt32(id);
...
function fetchCacheData(/** @type {IDBDatabase} */ db, fetch, data, onsuccess, onerror) {
  ...
  destinationPath ||= SAFE_HEAP_LOAD((((fetch) + (8)) >> 2) * 4, 4, 1);
...
function fetchLoadCachedData(db, fetch, onsuccess, onerror) {
  ...
  path ||= SAFE_HEAP_LOAD((((fetch) + (8)) >> 2) * 4, 4, 1);
...
function _emscripten_start_fetch(fetch, successcb, errorcb, progresscb, readystatechangecb) {
  ...
  if (requestMethod === "EM_IDB_STORE") {
    ...
    var size = SAFE_HEAP_LOAD((((fetch_attr) + (88)) >> 2) * 4, 4, 1);
...
function _emscripten_websocket_close(socketId, code, reason) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(48, 0, 1, socketId, code, reason);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
  reason ? UTF8ToString(reason) : undefined;
...
function _emscripten_websocket_delete(socketId) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(49, 0, 1, socketId);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_buffered_amount(socketId, bufferedAmount) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(50, 0, 1, socketId, bufferedAmount);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
  SAFE_HEAP_STORE(((bufferedAmount) >> 2) * 4, socket.bufferedAmount, 4);
  return 0;
}

function _emscripten_websocket_get_extensions(socketId, extensions, extensionsLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(51, 0, 1, socketId, extensions, extensionsLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_extensions_length(socketId, extensionsLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(52, 0, 1, socketId, extensionsLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
  if (!extensionsLength) return -5;
  SAFE_HEAP_STORE(((extensionsLength) >> 2) * 4, lengthBytesUTF8(socket.extensions) + 1, 4);
  checkInt32(lengthBytesUTF8(socket.extensions) + 1);
  return 0;
}

function _emscripten_websocket_get_protocol(socketId, protocol, protocolLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(53, 0, 1, socketId, protocol, protocolLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_protocol_length(socketId, protocolLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(54, 0, 1, socketId, protocolLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
  if (!protocolLength) return -5;
  SAFE_HEAP_STORE(((protocolLength) >> 2) * 4, lengthBytesUTF8(socket.protocol) + 1, 4);
  checkInt32(lengthBytesUTF8(socket.protocol) + 1);
  return 0;
}

function _emscripten_websocket_get_ready_state(socketId, readyState) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(55, 0, 1, socketId, readyState);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
  SAFE_HEAP_STORE(((readyState) >> 1) * 2, socket.readyState, 2);
  checkInt16(socket.readyState);
  return 0;
}

function _emscripten_websocket_get_url(socketId, url, urlLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(56, 0, 1, socketId, url, urlLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_url_length(socketId, urlLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(57, 0, 1, socketId, urlLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
  if (!urlLength) return -5;
  SAFE_HEAP_STORE(((urlLength) >> 2) * 4, lengthBytesUTF8(socket.url) + 1, 4);
  checkInt32(lengthBytesUTF8(socket.url) + 1);
  return 0;
}
...
function _emscripten_websocket_new(createAttributes) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(59, 0, 1, createAttributes);
...
function _emscripten_websocket_send_binary(socketId, binaryData, dataLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(60, 0, 1, socketId, binaryData, dataLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_send_utf8_text(socketId, textData) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(61, 0, 1, socketId, textData);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_set_onclose_callback_on_thread(socketId, userData, callbackFunc, thread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(62, 0, 1, socketId, userData, callbackFunc, thread);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
  socket.onclose = function(e) {
    ...
    SAFE_HEAP_STORE((eventPtr) + (4), e.wasClean, 1);
    checkInt8(e.wasClean), SAFE_HEAP_STORE((((eventPtr) + (6)) >> 1) * 2, e.code, 2);
...
function _emscripten_websocket_set_onerror_callback_on_thread(socketId, userData, callbackFunc, thread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(63, 0, 1, socketId, userData, callbackFunc, thread);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_set_onmessage_callback_on_thread(socketId, userData, callbackFunc, thread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(64, 0, 1, socketId, userData, callbackFunc, thread);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
  socket.onmessage = function(e) {
    ...
    checkInt32(len), SAFE_HEAP_STORE((eventPtr) + (12), isText, 1);
    checkInt8(isText), ((a1, a2, a3) => dynCall_iiii(callbackFunc, a1, a2, a3))(0, eventPtr, userData);
    _free(buf);
  };
  return 0;
}
...
function _emscripten_websocket_set_onopen_callback_on_thread(socketId, userData, callbackFunc, thread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(65, 0, 1, socketId, userData, callbackFunc, thread);
  // TODO:
  //    if (thread == 2 ||
...
function _fd_fdstat_get(fd, pbuf) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 0, 1, fd, pbuf);
  try {
    var rightsBase = 0;
...
var missingLibrarySymbols = [ "writeI53ToI64Clamped", "writeI53ToI64Signaling", "writeI53ToU64Clamped", "writeI53ToU64Signaling", "convertI32PairToI53", "convertI32PairToI53Checked", "convertU32PairToI53", "getTempRet0", "setTempRet0", "zeroMemory", "growMemory", "inetPton4", "inetNtop4", "inetPton6", "inetNtop6", "readSockaddr", "writeSockaddr", "emscriptenLog", "runMainThreadEmAsm", "jstoi_q", "listenOnce", "autoResumeAudioContext", "dynCallLegacy", "getDynCaller", "dynCall", "asmjsMangle", "alignMemory", "getNativeTypeSize", "addOnInit", "addOnPostCtor", "addOnPreMain", "STACK_SIZE", "STACK_ALIGN", "POINTER_SIZE", "ASSERTIONS", "uleb128Encode", "sigToWasmTypes", "generateFuncType", "convertJsFunctionToWasm", "getEmptyTableSlot", "updateTableMap", "getFunctionAddress", "addFunction", "removeFunction", "reallyNegative", "strLen", "reSign", "formatString", "intArrayToString", "AsciiToString", "stringToAscii", "UTF16ToString", "stringToUTF16", "lengthBytesUTF16", "UTF32ToString", "stringToUTF32", "lengthBytesUTF32", "JSEvents_requestFullscreen", "JSEvents_resizeCanvasForFullscreen", "registerRestoreOldStyle", "hideEverythingExceptGivenElement", "restoreHiddenElements", "setLetterbox", "softFullscreenResizeWebGLRenderTarget", "doRequestFullscreen", "requestPointerLock", "setCanvasElementSizeCallingThread", "setCanvasElementSizeMainThread", "setCanvasElementSize", "getCanvasSizeCallingThread", "getCanvasSizeMainThread", "getCanvasElementSize", "jsStackTrace", "getCallstack", "convertPCtoSourceLocation", "wasiRightsToMuslOFlags", "wasiOFlagsToMuslOFlags", "safeSetTimeout", "setImmediateWrapped", "safeRequestAnimationFrame", "clearImmediateWrapped", "registerPostMainLoop", "registerPreMainLoop", "getPromise", "idsToPromises", "makePromiseCallback", "ExceptionInfo", "findMatchingCatch", "Browser_asyncPrepareDataCounter", "arraySum", "addDays", "getSocketFromFD", "getSocketAddress", "FS_mkdirTree", "_setNetworkCallback", "heapObjectForWebGLType", "toTypedArrayIndex", "webgl_enable_ANGLE_instanced_arrays", "webgl_enable_OES_vertex_array_object", "webgl_enable_WEBGL_draw_buffers", "webgl_enable_WEBGL_multi_draw", "webgl_enable_EXT_polygon_offset_clamp", "webgl_enable_EXT_clip_control", "webgl_enable_WEBGL_polygon_mode", "emscriptenWebGLGet", "computeUnpackAlignedImageSize", "colorChannelsInGlTextureFormat", "emscriptenWebGLGetTexPixelData", "emscriptenWebGLGetUniform", "webglGetUniformLocation", "webglPrepareUniformLocationsBeforeFirstUse", "webglGetLeftBracePos", "emscriptenWebGLGetVertexAttrib", "__glGetActiveAttribOrUniform", "writeGLArray", "emscripten_webgl_destroy_context_before_on_calling_thread", "registerWebGlEventCallback", "runAndAbortIfError", "ALLOC_NORMAL", "ALLOC_STACK", "allocate", "writeStringToMemory", "writeAsciiToMemory", "demangle", "stackTrace" ];

missingLibrarySymbols.forEach(missingLibrarySymbol);

var unexportedSymbols = [ "run", "addRunDependency", "removeRunDependency", "out", "err", "callMain", "abort", "wasmMemory", "wasmExports", "HEAPF32", "HEAPF64", "HEAP8", "HEAPU8", "HEAP16", "HEAPU16", "HEAP32", "HEAPU32", "HEAP64", "HEAPU64", "writeStackCookie", "checkStackCookie", "writeI53ToI64", "readI53FromI64", "readI53FromU64", "INT53_MAX", "INT53_MIN", "bigintToI53Checked", "stackSave", "stackRestore", "stackAlloc", "ptrToString", "exitJS", "getHeapMax", "abortOnCannotGrowMemory", "ENV", "ERRNO_CODES", "strError", "DNS", "Protocols", "Sockets", "timers", "warnOnce", "readEmAsmArgsArray", "readEmAsmArgs", "runEmAsmFunction", "getExecutableName", "handleException", "keepRuntimeAlive", "runtimeKeepalivePush", "runtimeKeepalivePop", "callUserCallback", "maybeExit", "asyncLoad", "mmapAlloc", "HandleAllocator", "wasmTable", "noExitRuntime", "addOnPreRun", "addOnExit", "addOnPostRun", "getCFunc", "freeTableIndexes", "functionsInTableMap", "unSign", "setValue", "getValue", "PATH", "PATH_FS", "UTF8Decoder", "UTF8ArrayToString", "UTF8ToString", "stringToUTF8Array", "stringToUTF8", "lengthBytesUTF8", "intArrayFromString", "UTF16Decoder", "stringToUTF8OnStack", "writeArrayToMemory", "JSEvents", "registerKeyEventCallback", "specialHTMLTargets", "maybeCStringToJsString", "findEventTarget", "findCanvasEventTarget", "getBoundingClientRect", "fillMouseEventData", "registerMouseEventCallback", "registerWheelEventCallback", "registerUiEventCallback", "registerFocusEventCallback", "fillDeviceOrientationEventData", "registerDeviceOrientationEventCallback", "fillDeviceMotionEventData", "registerDeviceMotionEventCallback", "screenOrientation", "fillOrientationChangeEventData", "registerOrientationChangeEventCallback", "fillFullscreenChangeEventData", "registerFullscreenChangeEventCallback", "currentFullscreenStrategy", "restoreOldWindowedStyle", "fillPointerlockChangeEventData", "registerPointerlockChangeEventCallback", "registerPointerlockErrorEventCallback", "fillVisibilityChangeEventData", "registerVisibilityChangeEventCallback", "registerTouchEventCallback", "fillGamepadEventData", "registerGamepadEventCallback", "registerBeforeUnloadEventCallback", "fillBatteryEventData", "battery", "registerBatteryEventCallback", "UNWIND_CACHE", "ExitStatus", "getEnvStrings", "checkWasiClock", "doReadv", "doWritev", "initRandomFill", "randomFill", "emSetImmediate", "emClearImmediate_deps", "emClearImmediate", "promiseMap", "makePromise", "uncaughtExceptionCount", "exceptionLast", "exceptionCaught", "Browser", "getPreloadedImageData__data", "wget", "MONTH_DAYS_REGULAR", "MONTH_DAYS_LEAP", "MONTH_DAYS_REGULAR_CUMULATIVE", "MONTH_DAYS_LEAP_CUMULATIVE", "isLeapYear", "ydayFromDate", "SYSCALLS", "preloadPlugins", "FS_createPreloadedFile", "FS_modeStringToFlags", "FS_getMode", "FS_stdin_getChar_buffer", "FS_stdin_getChar", "FS_unlink", "FS_createPath", "FS_createDevice", "FS_readFile", "FS_root", "FS_mounts", "FS_devices", "FS_streams", "FS_nextInode", "FS_nameTable", "FS_currentPath", "FS_initialized", "FS_ignorePermissions", "FS_filesystems", "FS_syncFSRequests", "FS_readFiles", "FS_lookupPath", "FS_getPath", "FS_hashName", "FS_hashAddNode", "FS_hashRemoveNode", "FS_lookupNode", "FS_createNode", "FS_destroyNode", "FS_isRoot", "FS_isMountpoint", "FS_isFile", "FS_isDir", "FS_isLink", "FS_isChrdev", "FS_isBlkdev", "FS_isFIFO", "FS_isSocket", "FS_flagsToPermissionString", "FS_nodePermissions", "FS_mayLookup", "FS_mayCreate", "FS_mayDelete", "FS_mayOpen", "FS_checkOpExists", "FS_nextfd", "FS_getStreamChecked", "FS_getStream", "FS_createStream", "FS_closeStream", "FS_dupStream", "FS_doSetAttr", "FS_chrdev_stream_ops", "FS_major", "FS_minor", "FS_makedev", "FS_registerDevice", "FS_getDevice", "FS_getMounts", "FS_syncfs", "FS_mount", "FS_unmount", "FS_lookup", "FS_mknod", "FS_statfs", "FS_statfsStream", "FS_statfsNode", "FS_create", "FS_mkdir", "FS_mkdev", "FS_symlink", "FS_rename", "FS_rmdir", "FS_readdir", "FS_readlink", "FS_stat", "FS_fstat", "FS_lstat", "FS_doChmod", "FS_chmod", "FS_lchmod", "FS_fchmod", "FS_doChown", "FS_chown", "FS_lchown", "FS_fchown", "FS_doTruncate", "FS_truncate", "FS_ftruncate", "FS_utime", "FS_open", "FS_close", "FS_isClosed", "FS_llseek", "FS_read", "FS_write", "FS_mmap", "FS_msync", "FS_ioctl", "FS_writeFile", "FS_cwd", "FS_chdir", "FS_createDefaultDirectories", "FS_createDefaultDevices", "FS_createSpecialDirectories", "FS_createStandardStreams", "FS_staticInit", "FS_init", "FS_quit", "FS_findObject", "FS_analyzePath", "FS_createFile", "FS_createDataFile", "FS_forceLoadFile", "FS_createLazyFile", "FS_absolutePath", "FS_createFolder", "FS_createLink", "FS_joinPath", "FS_mmapAlloc", "FS_standardizePath", "MEMFS", "TTY", "PIPEFS", "SOCKFS", "tempFixedLengthArray", "miniTempWebGLFloatBuffers", "miniTempWebGLIntBuffers", "GL", "AL", "GLUT", "EGL", "GLEW", "IDBStore", "SDL", "SDL_gfx", "allocateUTF8", "allocateUTF8OnStack", "print", "printErr", "jstoi_s", "PThread", "terminateWorker", "cleanupThread", "registerTLSInit", "spawnThread", "exitOnMainThread", "proxyToMainThread", "proxiedJSCallArgs", "invokeEntryPoint", "checkMailbox", "webSockets", "WS", "WORKERFS", "Fetch", "fetchDeleteCachedData", "fetchLoadCachedData", "fetchCacheData", "fetchXHR" ];
...
function js_tracked_eval(code, size, debug) {
  const keys = Module["onRunTrackedJs"](UTF8ToString(code), debug);
  const error = keys === null;
  if (error) {
    SAFE_HEAP_STORE(size / HEAPU32.BYTES_PER_ELEMENT * 4, 0, 4);
    return 0;
  }
  const ptr = Module["_malloc"](keys.length * HEAPU32.BYTES_PER_ELEMENT);
  SAFE_HEAP_STORE(size / HEAPU32.BYTES_PER_ELEMENT * 4, keys.length, 4);
  HEAPU32.set(keys, ptr / HEAPU32.BYTES_PER_ELEMENT);
  return ptr;
}

function js_get_tracked_objects(keys_ptr, keys_n, sizes, statuses, objects_n, strings_n, all_byte_size) {
  const OK = 0;
  const BAD_KEY = 1;
  const NOT_STRING = 2;
  ...
  for (let i = 0; i < n; ++i) {
    ...
    SAFE_HEAP_STORE(statusPtr / HEAPU8.BYTES_PER_ELEMENT + i, status, 1);
    SAFE_HEAP_STORE((sizesPtr / HEAPU32.BYTES_PER_ELEMENT + i) * 4, byteSize, 4);
    allByteSize += byteSize;
  }
  const stringsPtr = Module["_malloc"](allByteSize * HEAPU8.BYTES_PER_ELEMENT);
  let currentStringsPtr = stringsPtr;
  for (let i = 0; i < n; ++i) {
    const status = SAFE_HEAP_LOAD(statusPtr / HEAPU8.BYTES_PER_ELEMENT + i, 1, 1);
    if (status !== OK) {
      continue;
    }
    const string = objects[i];
    const size = SAFE_HEAP_LOAD((sizesPtr / HEAPU32.BYTES_PER_ELEMENT + i) * 4, 4, 1);
    stringToUTF8(string, currentStringsPtr, size + 1);
    currentStringsPtr += size;
  }
  SAFE_HEAP_STORE(statuses / HEAPU32.BYTES_PER_ELEMENT * 4, statusPtr, 4);
  SAFE_HEAP_STORE(sizes / HEAPU32.BYTES_PER_ELEMENT * 4, sizesPtr, 4);
  SAFE_HEAP_STORE(objects_n / HEAPU32.BYTES_PER_ELEMENT * 4, n, 4);
  SAFE_HEAP_STORE(strings_n / HEAPU32.BYTES_PER_ELEMENT * 4, stringsN, 4);
  SAFE_HEAP_STORE(all_byte_size / HEAPU32.BYTES_PER_ELEMENT * 4, allByteSize, 4);
  return stringsPtr;
}
...
function assignWasmImports() {
  wasmImports = {
    ...
    /** @export */ emscripten_websocket_get_buffered_amount: _emscripten_websocket_get_buffered_amount,
    /** @export */ emscripten_websocket_get_extensions: _emscripten_websocket_get_extensions,
    /** @export */ emscripten_websocket_get_extensions_length: _emscripten_websocket_get_extensions_length,
    /** @export */ emscripten_websocket_get_protocol: _emscripten_websocket_get_protocol,
    /** @export */ emscripten_websocket_get_protocol_length: _emscripten_websocket_get_protocol_length,
    /** @export */ emscripten_websocket_get_ready_state: _emscripten_websocket_get_ready_state,
    /** @export */ emscripten_websocket_get_url: _emscripten_websocket_get_url,
    /** @export */ emscripten_websocket_get_url_length: _emscripten_websocket_get_url_length,
    ...
    /** @export */ js_get_tracked_objects,
    /** @export */ js_tracked_eval,
...
Module["_cast"] = createExportWrapper("cast", 2);

Module["_call"] = createExportWrapper("call", 2);

Module["_next_tracked_object_key"] = createExportWrapper("next_tracked_object_key", 0);

Module["_main"] = createExportWrapper("__main_argc_argv", 2);
...
Module["dynCall_vii"] = createExportWrapper("dynCall_vii", 3);

Module["dynCall_iii"] = createExportWrapper("dynCall_iii", 3);

Module["dynCall_jjj"] = createExportWrapper("dynCall_jjj", 3);

Module["dynCall_iiiiiiiii"] = createExportWrapper("dynCall_iiiiiiiii", 9);

Module["dynCall_iiiiii"] = createExportWrapper("dynCall_iiiiii", 6);

var dynCall_iiii = Module["dynCall_iiii"] = createExportWrapper("dynCall_iiii", 4);

Module["dynCall_iiiii"] = createExportWrapper("dynCall_iiiii", 5);

Module["dynCall_iiiiiii"] = createExportWrapper("dynCall_iiiiiii", 7);

var dynCall_ii = Module["dynCall_ii"] = createExportWrapper("dynCall_ii", 2);

Module["dynCall_viiii"] = createExportWrapper("dynCall_viiii", 5);

Module["dynCall_dd"] = createExportWrapper("dynCall_dd", 2);

Module["dynCall_ddd"] = createExportWrapper("dynCall_ddd", 3);

var dynCall_vi = Module["dynCall_vi"] = createExportWrapper("dynCall_vi", 2);

Module["dynCall_viii"] = createExportWrapper("dynCall_viii", 4);

Module["dynCall_iiiiiiii"] = createExportWrapper("dynCall_iiiiiiii", 8);

Module["dynCall_i"] = createExportWrapper("dynCall_i", 1);

Module["dynCall_jiji"] = createExportWrapper("dynCall_jiji", 4);

Module["dynCall_v"] = createExportWrapper("dynCall_v", 1);
